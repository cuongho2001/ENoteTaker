...
public class Classifier {
    ...
    //模型中输出向量的名称
    private static final String outputVectorName="output_vector";
    //模型中输出变量的名称
    ...
    static {
        ...
        Log.i("tensorflow","libtensorflow_inference.so库加载成功");
    }
    ...
    //欧式距离
    public double euclidean(float[] x1,float[] x2){
        double dist=0;
        double t=0;
    ...
    //余弦相似性
    public double cosine(float[] x1,float[] x2){
        double dist=0;
        double xy=0;
        double xx=0;
        double yy=0;
        ...
        dist=xy/(Math.sqrt(xx)*Math.sqrt(yy));
        return dist;
    }

    //获取特征抽取后的向量
    public float[] getVector(Bitmap bitmap){
        float[] inputdata=getPixels(bitmap);
        inferenceInterface.feed(inputName,inputdata,1,IMAGE_SIZE,IMAGE_SIZE,3);
        String[] outputNames=new String[]{outputName,probabilityName,outlayerName,outputVectorName};
        inferenceInterface.run(outputNames);
        float[] vector=new float[200];
        inferenceInterface.fetch(outputVectorName,vector);
        return vector;
    }
